...
<section>
	<a href="${pageContext.request.contextPath}/board/main?cNo=${cVo.cNo}" id="btnBack"></a>
	<div id="form_wrap">
		<h1>게시글 작성</h1>		
		<form action="regist" method="post">
			<fieldset>
				<c:if test="${tVo != null || dVo != null}">
					<c:if test="${dVo != null}">
						<p align="left">
							<input type="checkbox" name="kNo" value="${cVo.kVo.kNo}" id="kNotice"> 
							유치원 공지로 등록<label for="kNotice"></label>
						</p>
					</c:if>
					<p align="left">
						<input type="checkbox" name="bNotice" value="1" id="notice"> 
						반 공지로 등록<label for="notice"></label>
					</p>
				</c:if>
				<p>
					<input type="text" name="bTitle" placeholder="제목(50글자 이내)" data-msg="제목을 입력하세요" maxlength="50">
				</p>
				<p>
			...
			<input type="hidden" name="mVo.mNo" value="${dVo.mVo.mNo}${tVo.mVo.mNo}${pVo.mVo.mNo}">
			<input type="hidden" name="cVo.cNo" value="${cVo.cNo}">
			<input type="hidden" name="bNickname" value="${dVo.dNickname}${tVo.tNickname}${pVo.pNickname}">
